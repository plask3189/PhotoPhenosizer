...
def kickoff_window(): # The parameter is for the back button to not create a window, just create the frame_1 on the existing window.
    ...
    with open('user_project_directory.txt') as f:
...
def open_file(entry_box_for_file_path):
    ...
    with open("user_project_directory.txt",'w') as file: # Delete the contents of the user_project_directory.txt file
        pass
    ...
    with open('user_project_directory.txt', 'w') as f:
...
def get_tif_files(folder_selected_as_project_directory, frame_1, entry_box_for_file_path):
    project_dir = entry_box_for_file_path.get()
